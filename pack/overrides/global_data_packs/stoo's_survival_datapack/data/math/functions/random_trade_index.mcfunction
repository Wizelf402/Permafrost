...
###    > One of the two scores is then randomly transfered to the target score (*WTraderIndex)
...
scoreboard players operation @s WTraderIndex = @e[type=area_effect_cloud,name="stooRandomNumGen",distance=..1,limit=1] RNG_Floor
scoreboard players operation @s WTraderIndex %= @s RNG_Ceil
scoreboard players operation @s WTraderIndex += @s RNG_Floor
...
#scoreboard players reset * WTraderIndex
